...
lock, and local SQLite storage — while being written by someone who already knows
...
language *and* a new UI toolkit *and* a new BLE story simultaneously — and Phase 00
already proved it end to end (BLE scan/connect/GATT, background-safe patterns,
SQLite, a working installable APK). Re-litigating the framework now would mean
throwing that away.
...
`IDevice`/`ICharacteristic` — and it already has this specific treadmill's GATT 133
...
3. **Hand-rolled minimal wrapper** (just the handful of GATT operations this app
   ...
   write control point)
   - Pros: zero dependency, smallest possible surface — and this app's BLE needs
...
SQLite provider — its lifecycle tracks .NET's own, not a solo maintainer's spare
time.
...
rewritten, but the schema in `14-Database.md` stays valid either way — it's plain
SQLite under both). Lowest per-row overhead of the options, which matters at
...
exactly the kind of mechanical, low-judgement code a source generator should own,
...
2. **`CommunityToolkit.Maui`'s `FileSaver`** (the actual choice, as a secondary
   ...
   - When better: exactly as scoped — optional, not depended on.
...
3. **Hand-rolled SkiaSharp canvas**
   - Pros: total control, no charting-library dependency — and the actual
