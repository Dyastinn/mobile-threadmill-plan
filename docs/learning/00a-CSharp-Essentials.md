...
compiler checks it. Python or JavaScript let a variable hold anything, and
mistakes show up at runtime instead.
...
then never look at again. That's exactly the kind of boilerplate a language
...
not as a separate heap object, so there's no garbage-collector pressure. The
comment right above it in the real file explains why: `TreadmillSample` gets
created roughly once a second forever the app is connected, so avoiding a heap
allocation on that "hot path" actually matters. You don't need to deeply
understand GC pressure to work in this codebase. Just recognize `struct` as
"the lightweight one, used because something creates a lot of these, fast."
...
per-packet in FTMS, not per-device."* That means the treadmill might include speed
...
objects, not "a list of anything." That's what makes the compiler able to catch
